...
_NOTE:_ Although the compiler itself is licensed under the GPLv3, the code
generated by the compiler is not. Feel free to use the generated C or RISC code
however you see fit, although it comes with no warranty, express or implied.
...
- `-extra_runtime filename.c` causes an extra C source file (filename.c) to be
  ...
  `NATIVE` methods.
- `-use_int64` defines the C type of INTEGER to be `int64_t`. If this is not
  set, it uses the type `OBERON_INTEGER`. This is an `ifdef` (defaults to `int`)
